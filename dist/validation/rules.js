...
function add(diagnostics, code, path, message, fix) {
    ...
    diagnostics.push(Object.assign({ code, severity: 'error', path,
        message }, (fix ? { fix } : {})));
}
...
function checkRoot(form, diagnostics) {
    ...
    ['geometry', 'geometry_type', 'geometry_types'].forEach((key) => {
        if (own(form, key))
            checkGeometry(form[key], `/${key}`, diagnostics);
    });
    ...
    if (own(form, 'status_field'))
        checkStatusField(form.status_field, diagnostics);
    if (own(form, 'field_effects'))
        checkFieldEffects(form.field_effects, diagnostics);
}
function checkGeometry(value, path, diagnostics) {
    ...
    value.forEach((item, index) => {
        if (typeof item !== 'string' || !GEOMETRIES.has(item)) {
            add(diagnostics, 'geometry-value', appendPointer(path, index), 'geometry contains an unsupported value');
        }
    });
}
function checkStatusField(statusField, diagnostics) {
    ...
    if (Object.keys(statusField).length === 0
        || !own(statusField, 'enabled') || statusField.enabled !== true)
        return;
...
function checkElements(index, diagnostics, options = {}) {
    ...
    if (index.tooLarge)
        add(diagnostics, 'element-limit', '/elements', 'form exceeds the 1,400 flattened element limit');
...
function checkFieldLengths(element, path, diagnostics) {
    ...
    ['min_length', 'max_length'].forEach((key) => {
        ...
        if (!own(element, key) || !rubyTruthy(element[key]))
            return;
...
function checkNumericBounds(element, path, diagnostics) {
    ['min', 'max'].forEach((key) => {
        if (!own(element, key) || element[key] === null)
            return;
...
function checkTypeSpecific(element, path, diagnostics) {
    const type = own(element, 'type') ? element.type : undefined;
    if (own(element, 'geometry'))
        checkGeometry(element.geometry, `${path}/geometry`, diagnostics);
    if (type === 'Repeatable') {
        ['repeatable_geometry', 'geometry_types'].forEach((key) => {
            if (own(element, key))
                checkGeometry(element[key], `${path}/${key}`, diagnostics);
        });
    }
    if (type === 'ChoiceField')
        checkChoices(element, path, diagnostics);
    ...
    if (type === 'TextField') {
        ...
        if (element.numeric === true)
            checkNumericBounds(element, path, diagnostics);
    }
    ...
    if (own(element, 'ai_prompt')) {
        if (typeof element.ai_prompt !== 'string') {
            add(diagnostics, 'ai-prompt-type', `${path}/ai_prompt`, 'ai_prompt must be a string');
        }
        else {
...
function resolveReferences(form, index, diagnostics) {
    ...
    if (own(form, 'title_field_keys') && Array.isArray(form.title_field_keys)) {
        if (!form.title_field_keys.every((key) => typeof key === 'string')) {
            ...
            form.title_field_keys.forEach((key, i) => {
                if (!keyEntry(key)) {
                    add(diagnostics, 'title-field-reference', appendPointer('/title_field_keys', i), 'title field key does not resolve');
                }
            });
        }
    }
    index.elements.forEach((entry) => {
        ...
        ['visible_conditions', 'required_conditions'].forEach((property) => {
            ...
            element[property].forEach((condition, i) => checkCondition(condition, appendPointer(`${entry.path}/${property}`, i), entry, index, diagnostics));
        });
...
function checkCondition(condition, path, source, index, diagnostics) {
    ...
    if (key === '@status')
        return checkConditionOperator(condition, path, 'StatusField', diagnostics);
...
function checkPhotoFastFill(entry, index, diagnostics) {
    ...
    if (element.type !== 'PhotoField' || !nonblank(element.ai_prompt)
        || element.ai_prompt.length > 10000)
        return;
    const targets = element.ai_prompt.split('\n').map((target) => target.trim());
    while (targets.length && targets[targets.length - 1] === '')
        targets.pop();
    ...
    if (invalid) {
        add(diagnostics, 'fast-fill-reference', `${path}/ai_prompt`, 'each FastFill data name must resolve to a supported field in the same repeatable scope');
    }
}
function checkFieldEffects(value, diagnostics) {
    const path = '/field_effects';
    if (!present(value) || (isObject(value) && Object.keys(value).length === 0))
        return;
    if (!isObject(value)) {
        ...
        if (!own(effect, 'conditions') || !Array.isArray(effect.conditions)) {
            ...
            effect.conditions.forEach((condition, conditionIndex) => {
                if (!isObject(condition) || !nonblank(condition.field)
                    ...
                    add(diagnostics, 'field-effect-condition', appendPointer(`${effectPath}/conditions`, conditionIndex), 'field effect conditions require field and operator');
                }
            });
        }
        if (!own(effect, 'actions') || !Array.isArray(effect.actions)) {
            ...
            effect.actions.forEach((action, actionIndex) => {
                if (!isObject(action) || !nonblank(action.type)) {
                    add(diagnostics, 'field-effect-action', appendPointer(`${effectPath}/actions`, actionIndex), 'field effect actions require a type');
                }
            });
        }
    });
