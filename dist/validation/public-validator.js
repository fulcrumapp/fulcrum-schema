...
function codeFor(code) {
    ...
    if (/^(FORM|VALIDATION)\.[A-Z][A-Z0-9_]*$/.test(code))
        return code;
    ...
    try {
        ...
        if (structural || semantic) {
            ...
            if (index.tooDeep || index.cyclic || index.tooLarge) {
                if (structural)
                    mark('failures', 'structural', 'INPUT_LIMIT_EXCEEDED', '/elements');
                if (semantic)
                    mark('failures', 'semantic', 'INPUT_LIMIT_EXCEEDED', '/elements');
            }
...
function result(rawDiagnostics, coverage, versions) {
    if (rawDiagnostics.overflowed) {
        ...
        affectedChecks.forEach((check) => {
            ...
            if (!coverage.failures.some((entry) => (entry.check === check && entry.reason_code === 'INPUT_LIMIT_EXCEEDED'))) {
    ...
    const completed = coverage.completed.slice().sort((a, b) => ((order.has(a) ? order.get(a) : Number.MAX_SAFE_INTEGER)
