...
const { ELEMENT_TYPES, checkRoot, checkElements, resolveReferences, add, addWarning } = require('./rules');
...
function validate(request) {
    ...
    if (schemaUnsupported) {
        addWarning(diagnostics, 'unsupported-schema-version', '/schema_version', 'schema version is not supported; no version-specific rules were run');
    ...
    if (materialized.candidate !== undefined && !schemaUnsupported) {
        ...
        coverage.completed.push('element-structure', 'key-uniqueness', 'container-children', 'field-types');
