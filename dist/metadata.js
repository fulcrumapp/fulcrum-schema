...
var Metadata = function () {
  ...
  _createClass(Metadata, [{
    ...
    value: function buildStatements() {
      ...
      var systemTablesViewName = _utils2.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'tables_view');
      var systemColumnsName = _utils2.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'columns');
      var systemColumnsViewName = _utils2.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'columns_view');

      statements.push((0, _util.format)('CREATE TABLE IF NOT EXISTS %s (name text, alias text, type text, parent text, form_id text);', systemTablesName));

      statements.push((0, _util.format)('CREATE OR REPLACE VIEW %s AS SELECT alias AS name, type, parent, form_id FROM %s;', systemTablesViewName, systemTablesName));
