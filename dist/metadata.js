...
class Metadata {
  ...
  shouldEmitMetadata() {
    ...
    const newName = this.diff.newSchema && this.diff.newSchema.form ? this.diff.newSchema.form.name : null; // emit the metadata statements if there are already some changes being emitted or
  ...
  buildStatements() {
    ...
    const schemaChangesTableName = _utils.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'schema_changes');

    const systemTablesName = _utils.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'tables');

    const systemTablesViewName = _utils.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'tables_view');

    const systemColumnsName = _utils.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'columns');

    const systemColumnsViewName = _utils.default.tableName(this.options.schema, this.options.prefix, this.options.quote, 'columns_view');
    ...
    for (const view of this.newViews) {
      ...
      const viewType = view.type || view.table.type; // skip the _full and 'values' tables
      ...
      if (this.includeColumns) {
        for (let i = 0; i < view.columns.length; ++i) {
          const column = view.columns[i]; // statements.push(format('DELETE FROM %s WHERE table_name = %s AND name = %s;',
          //                        systemColumnsName,
          //                        pgvalue(viewName),
