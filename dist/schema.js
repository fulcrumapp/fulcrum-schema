...
const data_elements_1 = __importDefault(require("./data-elements"));
...
class Schema {
    constructor(form, columns, options) {
        ...
        this.elements = utils_1.default.flattenElements(this.form.elements, false);
        this.schemaElements = data_elements_1.default.find(this.elements);
        this.buildSchema();
    ...
    buildFormTable() {
        ...
        for (const column of this.columns.systemFormTableColumns) {
            const formColumn = (0, lodash_1.clone)(column);
    ...
    buildValuesTable() {
        const table = new Table((0, util_1.format)('form_%s_values', this.form.row_id), null, { type: 'values', alias: this.alias('values'), form_id: this.form.id });
        for (const column of this.columns.systemValuesTableColumns) {
            const valueColumn = (0, lodash_1.clone)(column);
    ...
    buildRepeatableTable(parentTable, element) {
        const table = new Table(this.formTable.id + '_' + element.key, null, { type: 'repeatable', parent: parentTable, element: element, alias: this.alias(element.data_name), form_id: this.form.id });
        for (const column of this.columns.systemRepeatableTableColumns) {
            const attrs = (0, lodash_1.clone)(column);
        ...
        for (const column of table.columns) {
            ...
            if (!columnNames[alias]) {
                if (column.type === 'boolean') {
                    const projection = `${column.name} IS NOT NULL AND ${column.name} = 't' AS ${utils_1.default.escapeIdentifier(alias)}`;
    ...
    buildIndexes() {
        for (const table of this.tables) {
            ...
            for (const index of indexDefinitions) {
                const indexDefinition = (0, lodash_1.clone)(index);
    ...
    addMediaElement(table, element) {
        ...
        const clause = (0, util_1.format)('WHERE key = \'%s\'', value);
        const filter = {
            key: value
        };
        const alias = {
    ...
    addRecordLinkElement(parentTable, element) {
        ...
        const value = element.key.replace(/'/g, "''");
        const clause = (0, util_1.format)('WHERE key = \'%s\'', value);
        const view = new View(this.formTable.id + '_' + element.key + '_view', null, this.valuesTable, { type: 'link', element: element, clause: clause, alias: this.alias(element.data_name) });
    ...
    addRepeatableElement(parentTable, element) {
        ...
        const elements = utils_1.default.flattenElements(element.elements, false);
        const childElements = data_elements_1.default.find(elements);
