  ...
  Object.keys(oldLeaves).sort().forEach((key) => {
    ...
    if (newLeaves[key] && oldType !== newType) {
      ...
      add(
        diagnostics,
        'leaf-type-change',
        `${newLeaves[key].path}/type`,
        `existing leaf at ${oldLeaves[key].path} (${previousType}) cannot change to `
          + `(${effectiveType}) at ${newLeaves[key].path}`,
        'use a new key for a different field type'
      );
    }
  });
}
