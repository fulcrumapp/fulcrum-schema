...
describe("v4 to v5 form schema generator", () => {
  it("builds a schema diff for a new v5 form", () => {
    const sql = compareFormSchemas(null, v5Form, OPTIONS);

    const expected = fixture("v5-create-form.sql");

    toSQL(sql).should.eql(expected);
  });

  it("builds a schema diff for a v2 to v5 form", () => {
    const sql = compareFormSchemas(v2Form, v5Form, OPTIONS);

    const expected = fixture("v5-update-form-from-v2.sql");

    toSQL(sql).should.eql(expected);
  });

  it("builds a schema diff for a v4 to v5 form", () => {
    const sql = compareFormSchemas(v4Form, v5Form, OPTIONS);

    const expected = fixture("v5-update-form.sql");

    toSQL(sql).should.eql(expected);
  });

  it("builds a schema diff for a deleted v5 form", () => {
    const sql = compareFormSchemas(v5Form, null, OPTIONS);
