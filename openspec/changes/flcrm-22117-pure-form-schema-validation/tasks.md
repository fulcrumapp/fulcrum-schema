...
- [x] 1.1 Record the exact approved Rails source revision and create a rule matrix classifying each selected rule as supported, provisional, contextual, mutating, or unsupported; verify every ruleset entry cites a source path/fixture and no unsupported rule is advertised as complete.
- [x] 1.2 Add minimal valid create and previous/update fixtures plus adversarial deltas for every supported rule; verify fixtures preserve `false`, zero, `null`, empty, and omitted values distinctly.
...
- [x] 3.1 Implement root/common/container structural rules and globally unique keys; verify each valid/invalid/nested fixture produces the expected path-addressed diagnostics.
...
- [x] 5.1 Reconcile the approved FLCRM-22116 method/request/result names, outcomes, diagnostics, versions, coverage IDs, limits, and update semantics; verify the approval reference is recorded in the spec and PR evidence.
- [x] 5.2 Reconcile the internal adapter and fixtures to the approved shared v1 contract and update OpenSpec plus `.agents/specs/active-spec.md`; verify no provisional field, enum, or code remains in the public contract.
